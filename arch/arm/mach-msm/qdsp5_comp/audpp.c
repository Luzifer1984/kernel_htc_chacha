			    ...
			    void (*getevent)(void *ptr, size_t len))
{
	...
	if (id == AUDPP_MSG_AVSYNC_MSG) {
		MM_AUD_INFO("audpp: AUDPP_MSG_AVSYNC_MSG received\n");
	...
	case AUDPP_MSG_STATUS_MSG: {
		unsigned cid = msg[0];
		MM_AUD_INFO("audpp: status %d %d %d\n", cid, msg[1], msg[2]);
	...
	case AUDPP_MSG_CFG_MSG:
		...
		audpp_broadcast(audpp, id, msg);
			...
			MM_AUD_ERR("audpp: invalid config msg %d\n", msg[0]);
		break;
	case ADSP_MESSAGE_ID:
		MM_AUD_INFO("audpp: module enabled\n");
		break;
	default:
...
#if 0
		MM_AUD_INFO("set eq\n");
		MM_AUD_INFO("eq.num_bands = 0x%04x\n", audpp->eq.num_bands);
		for (i = 0; i < 132; i++) \
			MM_AUD_INFO("eq_params[%d] =0x%04x\n", i,
